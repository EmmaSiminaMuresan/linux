...
#define ADI_EMU_RD_MASK		BIT(7)		// 0x80
#define ADI_EMU_ADDR_MASK	GENMASK(14, 8)	// mask bits 14 to 8 (15 needs to be 0 (R/W~))
...
static int adi_emu_spi_read(struct adi_emu_state *st, u8 reg, u8 *val){
	...
	tx |= ADI_EMU_RD_MASK;		// mask for 7th bit(R/W~)
	tx |= reg;
...
static int adi_emu_spi_write(struct adi_emu_state *st, u8 reg, u8 val){
	...
	put_unaligned_be16(msg, &tx);		// function to fix endianness miss-match
...
static int adi_emu_reg_access(struct iio_dev *indio_dev, unsigned reg,
