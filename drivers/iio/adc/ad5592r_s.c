...
static int ad5592r_s_spi_read_adc(struct ad5592r_s_state *st, u8 chan, int *val)
{
	...
	addr = (tmp >> 12);
	dev_info(&st->spi->dev, "req chan = 0x%x; recv chan = 0x%x", chan,
		 addr);
			      ...
			      int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		if (ret) {
			dev_err(&st->spi->dev, "failed at read chan %d",
				chan->channel);
			return ret;
		}
		return IIO_VAL_INT;
...
static int ad5592r_s_init(struct iio_dev *indio_dev)
{
	...
	ret = ad5592r_s_spi_write(st, AD5592R_ADC_CONFIG,
				 AD5592R_ADC_CONFIG_MASK);
	
	if (ret) {
		dev_err(&st->spi->dev, "Failed at configuring ADC in pins");
		return ret;
	}
