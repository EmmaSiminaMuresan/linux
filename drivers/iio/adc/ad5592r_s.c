...
static int ad5592r_s_spi_nop(struct ad5592r_s_state *st,
			     u16 *val)
{
...
static int ad5592r_s_spi_read_adc(struct ad5592r_s_state *st,
			          u8 chan,
			          int *val)
{
	u16 tx = 0;
	...
	addr = (addr >> 12);
	dev_info(&st->spi->dev, 
		 "req chan = 0x%x; recv chan =  0x%x", chan, addr);

	if(addr != chan) {
		dev_err(&st->spi->dev, 
			"requested channel does not match read channel");
		return -EINVAL;
	}
			      ...
			      long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		if(ret) {
			dev_err(&st->spi->dev, 
				"Failed at read channel %d", chan->channel);
			return ret;
		}
		return IIO_VAL_INT;
