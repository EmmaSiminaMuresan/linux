...
#define AD5592R_S_MASK_REF_EN       BIT(6)
...
static int ad5592r_s_spi_read_adc(struct ad5592r_s_state *st, u8 chan, int *val)
{
        ...
        msg |= FIELD_PREP(AD5592R_S_ADDR_MASK, AD5592R_S_ADC_SEQ);
		...
		addr = (addr >> 12);
 		dev_err(&st->spi->dev, "req chan = 0x%x; recv chan = 0x%x", chan, addr);
		
		if (addr != chan)  
		{
			dev_err(&st->spi->dev, "req channel doesn'ttrack the good channe");
		}
